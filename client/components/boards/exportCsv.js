Template.boardMenuPopup.events({
  'click .pop-over-list-a'() {
    ...
    function JSONToCSV(JSONData, ReportName, ShowLabel) {
      ...
      const ifdo = document.createElement('iframe');
      ifdo.id = 'ifdiframe';
      ifdo.name = 'ifdiframe';
      ...
      if (window.navigator.userAgent.indexOf('Firefox') >= 1) {
        ...
        //支持IE9、IE8;  IE7及以下暂不支持，因为不支持JSON
        const ifd = document.getElementById('ifdiframe').contentDocument;
    ...
    function getcont(data, name) {
      let dataFilter = [],
        custom = [];
      custom = customFields.filter(
        item => item.name.substring(item.name.indexOf('.') + 1) == name,
      );
      if (custom.length > 0) {
    ...
    function getFactFinishTime(cardId, activities, name) {
      const filterData = activities.filter(
        item =>
          item.activityType == 'moveCard' &&
          item.cardId == cardId &&
          item.listName == name,
      );
    ...
    //获取数据
    HTTP.get(
      FlowRouter.path('/api/boards/:boardId/export', params, queryParams),
      {},
      function(error, response) {
        if (error) {
          console.log(error);
          ...
          // 选中项放入数组
          const dataArr = [];
          const listCard = $('.list-body .minicards .minicard-wrapper');
          let lscards = [];
            ...
            if (!isNaN(i)) {
              if (listCard[i].classList.value.indexOf('is-checked') != '-1') {
                listArr(
                  lscards,
                  jsonData[0],
                  listCard[i].pathname.substring(
                    listCard[i].pathname.lastIndexOf('/') + 1,
                    listCard[i].pathname.length,
                  ),
                );
              }
            }
          ...
          for (var i in lscards) {
            if (lscards[i].archived == false) {
              ...
              const jsons = {
                ...
                工作内容:
                  lscards[i].description != '' && lscards[i].description != null
                    ? lscards[i].description
                    : '未填写工作内容',
                ...
                实际完成时间: getFactFinishTime(
                  lscards[i]._id,
                  jsonData[0].activities,
                  '已完成',
                ),
          ...
          const GridData = JSON.parse(JSON.stringify(dataArr));
  ...
  'click .printCards'() {
    ...
    function preview(oper) {
      if (oper < 10) {
        ...
        const header_quick_access = document.getElementById(
          'header-quick-access',
        );
        const pop_over = document.getElementById('pop-over');
